...
namespace WebCostosPresupuestos
{
    public partial class WebComparacionesLineaBase : System.Web.UI.Page
    {
        ...
        public List<string> Lista
        {
            get
            {
                if (HttpContext.Current.Session["ListaLineaBase"] == null)
                {
                    HttpContext.Current.Session["ListaLineaBase"] = new List<string>();
                }
                return HttpContext.Current.Session["ListaLineaBase"] as List<string>;
            }
            set
            {
                HttpContext.Current.Session["ListaLineaBase"] = value;
            }
                ...
                for (int currentRow = 0; currentRow < gridview1.Rows.Count; currentRow++)
                {
                    ...
                    for (int cells = 0; cells < gridview1.Rows[currentRow].Cells.Count; cells++)
                    {
                        ...
                        if (rowToCompare.Cells[cells].Text != rowToCompare2.Cells[cells].Text)
                        {
                           ...
                           /* ((Label)GridView1.Rows[currentRow].FindControl("Label1")).Text = (Convert.ToDecimal(rowToCompare.Cells[40].Text) - Convert.ToDecimal(rowToCompare2.Cells[40].Text)).ToString();

                            if (((Label)GridView1.Rows[currentRow].FindControl("Label1")).Text != "Label")
                            {

                                /// ((Label)GridView1.Rows[currentRow].FindControl("Label1")).Text = "0";
                                /// }
                                /// else
                                /// {
                                grdTotal = grdTotal + Convert.ToDecimal(((Label)GridView1.Rows[currentRow].FindControl("Label1")).Text);
                            }*/
            ...
            try
            {
             //   TxtLineaBase.Text = "";
               // LblId.Text = "";
                ...
                {
                   // TxtLineaBase.Text = drvSql["Fecha"].ToString();
                   // LblId.Text = drvSql["Id"].ToString();
                }

                //Session["IdFecha"] = LblId.Text.ToString();
        ...
        public void Encontrar() {
            ...
            foreach (DataRow row2 in dt.Rows)
            {
                ...
                Llenar(ac, valor);
                //nuevoTxt2.ID = ac;
                //nuevoTxt2.Text = valor;
                //Panel1.Controls.Add(nuevoTxt2);
